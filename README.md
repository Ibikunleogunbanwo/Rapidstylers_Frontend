# RapidStylers Marketplace Web App

RapidStylers is a Canadian beauty services marketplace that connects clients with vetted mobile beauty professionals for appointments at home, at an event, or wherever the client needs care.

This repository is the React web app for the public site, customer dashboard, stylist onboarding and dashboard, and admin console. It is built to feel polished on the outside, while the platform underneath connects to a hardened Spring Boot backend with JWT role checks, Redis rate limiting, Kafka and outbox workflows, Cloudinary signed uploads, Google Places, and booking prices that account for travel distance.

## Product Preview

![RapidStylers professional landing section](src/assets/images/stylist%202.png)
...
- Booking prices are owned by the backend, including a stylist set base earning for the first 15 km and an additional travel calculation above 15 km.
- Login attempts are audited and rate limited through Redis instead of local memory.
- Notifications are being moved behind Kafka and outbox processing so email, SMS, booking, and payment side effects do not block core user flows.
- Cloudinary uploads are signed by the backend, limited to approved folders, and rate limited to reduce abuse.
- APIs separate `CUSTOMER`, `STYLER`, and `ADMIN` actions through role checks.
...
The current MVP pass focused on turning the prototype into a more production ready system:

| Area | Recent change |
| --- | --- |
| Booking economics | Added support for stylist/vendor base earning within 15 km and extra charges above 15 km. |
| Messaging architecture | Added Kafka and outbox direction so notification, email, SMS, booking, and payment events can be processed asynchronously. |
| Runtime infrastructure | Dockerized Redis and Kafka with ports chosen to avoid conflict with the AfroChow local stack. |
| Security | Added Redis rate limiting, login attempt audit records, stricter CORS configuration, JWT role protection for sensitive endpoints, and guarded Cloudinary upload signatures. |
| UX polish | Fixed the stylist registration CTA so it navigates to the stylist signup page and behaves like a real responsive button. |
| Deployment | Prepared the Cloudflare and Vercel domain flow for `rapidstylers.ca` and `www.rapidstylers.ca`, with backend expected at `api.rapidstylers.ca`. |
...
3. The backend API key filter rejects unknown clients, while the JWT filter protects endpoints by role.
...
hooks/local/            Redux store, reducers, and local workflows
hooks/remote/           Axios client, API service methods, API tests
pages/admin/            admin login, categories, blog, stylists, operations
pages/generalPages/     landing, auth, search, blog, stylist profiles
pages/styler/           onboarding, dashboard, profile, appointments
pages/users/            customer dashboard, booking, account, support
...
| Login fails without a useful message | Confirm the page handles application `statusCode: "400"` responses, not only HTTP errors. |
...
RapidStylers demonstrates product thinking across the full stack:
...
- Event based design using Kafka and the transactional outbox pattern.
- Deployment awareness across Vercel, Cloudflare, Docker, Redis, Kafka, MySQL, and a Spring Boot API hosted on a VPS.
- Practical UX work, including responsive public pages, onboarding flows, dashboards, fallback content, and error states users can understand.
